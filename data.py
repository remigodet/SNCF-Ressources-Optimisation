...
NAME = "instance_WPY_realiste_corrigee2.xlsx"
data = {}
p_bar = tqdm(range(6), desc="LOADING DATA")
data["taches_df"] = pd.read_excel(
    f"./Data SNCF/{NAME}", sheet_name="Taches humaines")
p_bar.update(1)
p_bar.refresh()
data["chantiers_df"] = pd.read_excel(
    f"./Data SNCF/{NAME}", sheet_name="Chantiers")
p_bar.update(1)
p_bar.refresh()
data["machines_df"] = pd.read_excel(
    f"./Data SNCF/{NAME}", sheet_name="Machines")
p_bar.update(1)
p_bar.refresh()
data["sillons_df"] = pd.read_excel(f"./Data SNCF/{NAME}", sheet_name="Sillons")
p_bar.update(1)
p_bar.refresh()
data["correspondances_df"] = pd.read_excel(
    ...
    if row.LDEP != "NC":
        ...
        try:
            ...
            try:
                ...
                try:
                    a = data["sillons_df"][(
                        data["sillons_df"]["n°TRAIN"] == row["n°TRAIN"])]["train_id"].iloc[0]
                except:
                    print(row["n°TRAIN"])
                    print(row.JDEP)
...
p_bar.update(1)
p_bar.refresh()
data["sillons_df"]["HARR"] = data["sillons_df"].apply(
    lambda row: str(row["HARR"])[:5], axis=1)
data["sillons_df"]["HDEP"] = data["sillons_df"].apply(
    lambda row: str(row["HDEP"])[:5], axis=1)
data["sillons_df"]["train_id"] = data["sillons_df"].apply(lambda row: str(
    row["n°TRAIN"])+str(row.JDEP)+str(row.HDEP)+str(row.HARR)+str(row.JARR), axis=1)
data["correspondances_df"]["train_id"] = data["correspondances_df"].apply(
    lambda row: get_id(row), axis=1)
p_bar.update(1)
p_bar.refresh()
