...
def generate_contraintes(m, dataframes, dico):
    ...
    for chantier in set(taches_df["Chantier"].values):
        chantier_cycles[chantier,"start"] = taches_df[taches_df["Chantier"]==chantier][taches_df.Ordre == taches_df[taches_df["Chantier"]==chantier].Ordre.max()]["Type de tache humaine"].iloc[0]
        chantier_cycles[chantier,"end"] = taches_df[taches_df["Chantier"]==chantier][taches_df.Ordre == taches_df[taches_df["Chantier"]==chantier].Ordre.min()]["Type de tache humaine"].iloc[0]
    ...
    def add_occupation_constr(chantier, tache_debut):
        def b(minute_i,minute_j):
            ...
            m.addConstr((b == 1) >> (minute_i<=minute_j), name="indicator_constr1")
            m.addConstr((b == 0) >> (minute_i>=minute_j + 0.5), name="indicator_constr2")
            return b
